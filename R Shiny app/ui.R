             ...
             tabPanel("Kmeans",
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          br(),
                          p("Select the PCA components and algorithm to perform Kmeans"),
                          fluidRow(
                            column(4,uiOutput("km_pcs_x")),
                            column(4,uiOutput("km_pcs_y")),
                            column(4,selectInput("kmalgo", label = "Algorithm :", 
                          ...
                          sliderInput('clusters', 'Cluster count', 3, min = 2, max = 20),
                          numericInput("nstart", label = "Random seed:", value = 17),
                          tags$hr(),
             ...
             tabPanel("GMM",
                      sidebarLayout(
                        sidebarPanel(
                          strong("INPUT GMM"),
                          br(),
                          p("Select the PCA components to perform GMM"),
                          fluidRow(
                            column(6, uiOutput("gmm_pcs_x")),
                            column(6, uiOutput("gmm_pcs_y"))
                          )
                        ),# sidebar panel
                        mainPanel(
                          tabsetPanel(
                      ...
                      tabPanel("Spectral clustering",
                               sidebarLayout(
                                 sidebarPanel(
                                   ...
                                   br(),
                                   p("Select the PCA components to perform spectral clustering"),
                                   fluidRow(
                                     column(6,uiOutput("specc_pcs_x")),
                                     column(6,uiOutput("specc_pcs_y"))
                                   ),
                                   sliderInput("specc_centers", "Indicate the desired number of centres :", min=2, max=20, value=3),
                       ...
                       tabPanel("Affinity propagation",
                                sidebarLayout(
                                  sidebarPanel(
                                      ...
                                      br(),
                                      p("Select the PCA components to perform affinity propagation"),
                                      p("affinity propagation does not require the number of clusters to be determined or estimated before running the algorithm"),
                                      fluidRow(
                                        column(6,uiOutput("ap_pcs_x")),
                                        column(6,uiOutput("ap_pcs_y"))
                                      )
                                    ),# sidebar panel
                                    mainPanel(
                      ...
                      tabPanel("Ensemble Clustering",
                               sidebarLayout(
                                  sidebarPanel(
                                    ...
                                    br(),
                                    p("Select the PCA components to perform consensus clustering"),
                                    fluidRow(
                                      column(6,uiOutput("ec_pcs_x")),
                                      column(6, uiOutput("ec_pcs_y"))
                                    ),
                                    tags$hr(),
                                    checkboxGroupInput("nk_check", "Clusters :", choices = c(2:19), selected = c(2:4), inline=TRUE),
